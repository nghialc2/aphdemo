...
export const useNotebooks = () => {
  ...
  const createNotebook = useMutation({
    mutationFn: async (notebookData: { title: string; description?: string }) => {
      ...
      // Use the user ID from the main auth, but insert into InsightsLM database
      // This will work if RLS is disabled or modified
      const { data, error } = await supabase
        .from('notebooks')
        .insert({
          title: notebookData.title,
          description: notebookData.description,
          user_id: user.id, // This is the user ID from main database
