...
export const AuthProvider = ({ children }: AuthProviderProps) => {
  ...
  const updateAuthState = (newSession: Session | null) => {
    console.log('InsightsLM AuthContext: Updating auth state:', newSession?.user?.email || 'No session');
  ...
  const clearAuthState = () => {
    console.log('InsightsLM AuthContext: Clearing auth state');
    setSession(null);
    setUser(null);
    setError(null);
  };
  ...
  const signOut = async () => {
    try {
      ...
      try {
        ...
        console.error('InsightsLM AuthContext: Failed to clear local session:', localError);
      }
    }
  };
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(
      (event, newSession) => {
        ...
        console.log('InsightsLM AuthContext: Auth state changed:', event, newSession?.user?.email || 'No session');
    ...
    const initializeAuth = async () => {
      try {
        console.log('InsightsLM AuthContext: Initializing auth...');
        
        // Get initial session
        const { data: { session: initialSession }, error: sessionError } = await supabase.auth.getSession();
        
        if (sessionError) {
          console.error('InsightsLM AuthContext: Error getting initial session:', sessionError);
          ...
          if (sessionError.message.includes('session_not_found') || 
            ...
            console.log('InsightsLM AuthContext: Session not found on server, clearing local session');
            await supabase.auth.signOut({ scope: 'local' });
            if (mounted) {
              clearAuthState();
              setLoading(false);
        ...
        if (mounted) {
          console.log('InsightsLM AuthContext: Initial session:', initialSession?.user?.email || 'No session');
          updateAuthState(initialSession);
          setLoading(false);
        }
      } catch (err) {
        console.error('InsightsLM AuthContext: Auth initialization error:', err);
        if (mounted) {
